...
let alertProps = { queue: [] };
const notifyProps = { quadrants: { topRight: [], bottomRight: [], bottomLeft: [], topLeft: [] }};
...
const chatgpt = {
    ...
    alert: function(title: string, msg: string, btns: any, checkbox: any, width: number) {
    ...
    // * Spaces are inserted into button labels by parsing function names in camel/kebab/snake case
        ...
        function destroyAlert() {
            ...
            const alertProps = JSON.parse(localStorage.alertProps);
            ...
            if (alertProps.queue.length > 0) {
                const nextAlert = document.getElementById(alertProps.queue[0]);
                setTimeout(() => { nextAlert.style.display = 'flex'; }, 500 );
            }
        }

        function keyHandler(event) {
            const dismissKeys = [13, 27, 32]; // enter/esc/space
            if (dismissKeys.includes(event.keyCode)) {
                for (const i = 0; i < alertProps.queue.length; i++) { // look to handle only if triggering alert is active
                    const alert = document.getElementById(alertProps.queue[i]);
    ...
    autoRefresh: {
        activate: function(interval) {
            ...
            function scheduleRefreshes(interval) {
                const randomDelay = Math.max(2, Math.floor(Math.random() * 21 - 10)); // set random delay up to ±10 secs
                autoRefresh.isActive = setTimeout(() => {
                    const refreshFrame = document.querySelector('#refresh-frame');
    ...
    getLastResponseDiv: function() {
        const responseDivs = document.querySelectorAll('main > div > div > div > div > div[class*=group] p');
        return responseDivs.length ? responseDivs[responseDivs.length - 1] : '';
    },

    getResponseDivs: function() {
    ...
    notify: function(msg, position, notifDuration, shadow) {
        ...
        const thisQuadrantDivIDs = notifyProps.quadrants[notificationDiv.quadrant];
        if (thisQuadrantDivIDs.length > 1) {
            try { // to move old notifications
                const divsToMove = thisQuadrantDivIDs.slice(0, -1); // exclude new div
                for (const j = 0; j < divsToMove.length; j++) {
    ...
    send: function(msg, method='') {
        ...
        const delaySend = setInterval(function() {
